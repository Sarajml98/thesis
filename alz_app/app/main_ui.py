...
if run_all:
    if not data_root:
        ...
        # ⚡ Bolt: clear cache when re-running all analyses to ensure fresh data
        st.cache_data.clear()
...
def load_all_results():
    """⚡ Bolt Optimization: Load results from disk and cache them.
    Also converts prediction lists to dictionaries for O(1) lookups.
    """
    results = {}
    ...
    # If aggregate is empty or missing, try per-module files
    if not results:
        for name in ["mri_pet", "eeg", "adni", "tadpole", "proteomics"]:
            ...
            if summary_path.exists():
                try:
                    ...
                    # Try to attach predictions from CSV
    ...
    # ⚡ Bolt Optimization: Ensure all predictions are dictionaries for O(1) lookups
...
# If no results in session, try to load using cached function
if not results:
