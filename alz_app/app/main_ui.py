...
# Cache the expensive file I/O operations
@st.cache_data
def load_previous_results():
    """Load previous results from outputs folder, trying aggregate file first."""
    results = {}
    ...
    if agg.exists():
        try:
            ...
            st.warning(f"Could not load aggregate results: {e}")
            results = {}

    # if aggregate failed or doesn't exist, try to load per-module summary files
...
# If no results in session, try to load from outputs folder
if not results:
