...
@st.cache_data
def load_results():
    """Load results from output files, caching the result.

    This function reads all summary and prediction files from the `outputs`
    directory. Using @st.cache_data prevents expensive file I/O on every
    UI interaction.
    """
    results = {}
    import json
    ...
    # prefer aggregate file if it exists
    if agg.exists():
        try:
    ...
    # if aggregate file is missing, try to load per-module summary files
...
# If a run was just completed, use its results. Otherwise, load from disk.
