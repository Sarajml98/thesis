...
default_data = str(BASE_DIR / "data")
...
@st.cache_data
def load_results():
    """Load results from disk and optimize for lookup. Cached to avoid redundant I/O."""
    results = {}
                ...
                try:
                    ...
                    if preds_path.exists():
                        ...
                        with open(preds_path, "r", encoding="utf-8") as pf:
                            ...
                            for row in reader:
                                prob_val = float(row.get("probability", 0.0))
                                rows.append({
    ...
    # Post-process: add predictions_map for O(1) performance
    for name, summary in results.items():
        if "predictions" in summary and "predictions_map" not in summary:
...
if run_all:
    if not data_root:
        ...
        # Save final summary
        out_file = BASE_DIR / "outputs" / "final_summary.txt"
...
# If no results in session, try to load via cached function
if not results:
    results = load_results()
    if results:
        st.session_state["last_results"] = results
        st.info("Loaded previous results from disk (cached).")
    else:
...
for r in results.values():
    # Bolt: Optimized subject set construction using predictions_map (O(M))
    if "predictions_map" in r:
        subjects_set.update(r["predictions_map"].keys())
    elif "predictions" in r:
        subjects_set.update(p.get("subject_id") for p in r["predictions"])
if not subjects_set:
