...
def load_cached_results():
    """Loads results from the outputs directory, trying aggregate first, then per-module files."""
    # This is an expensive file I/O operation that will now be cached.
    # Reading multiple JSON and CSV files from disk on every interaction was
    # a key bottleneck. Caching this significantly improves UI responsiveness
    ...
    if agg_path.exists():
        try:
            ...
            pass  # Fallback to per-module files if JSON is corrupt or unreadable

    # Fallback or primary loading of per-module files
    for name in ["mri_pet", "eeg", "adni", "tadpole", "proteomics"]:
        ...
        try:
            ...
            if preds_path.exists():
                rows = []
                with preds_path.open("r", encoding="utf-8") as pf:
            ...
            if name in results:
                del results[name]  # Clean up partial load
            continue
...
if "last_results" not in st.session_state:
    st.session_state["last_results"] = load_cached_results()
