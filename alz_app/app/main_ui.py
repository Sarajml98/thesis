...
# ⚡ Bolt: Relocated imports to top-level to avoid redundant module lookups
...
def load_results_from_disk():
    """⚡ Bolt: Cached I/O for loading results from disk."""
    results = {}
...
@st.cache_data
def get_subjects_list(results):
    """⚡ Bolt: Optimized subject list building from prediction dictionaries."""
    subjects_set = set()
...
if not results:
    ...
    if results:
        st.info("Loaded previous results (cached).")
        st.session_state["last_results"] = results
    else:
        st.info("No results available. Run 'Run All Analyses' to produce results, or load previous results.")

# build subject list (cached)
subjects_list = get_subjects_list(results)
