...
def load_previous_results():
    """
    BOLT ⚡: This function is cached to prevent re-reading files from disk on every UI interaction.
    It loads all result summaries and predictions from the `outputs/` directory.
    """
    results = {}
    import json
    import csv
    ...
    if agg.exists():
        try:
            with open(agg, "r", encoding="utf-8") as f:
                results = json.load(f)
            # BOLT ⚡: UI elements should not be in cached functions. This is now handled below.
            return results
        except Exception:
