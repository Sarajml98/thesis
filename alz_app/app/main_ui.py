...
# ⚡ Bolt: Caching file I/O to improve performance.
# WHAT: This function loads all result files (JSON, CSV) from the outputs directory.
# WHY: The previous implementation read these files on every UI interaction, causing
#      unnecessary disk I/O and slowing down the app.
# IMPACT: Caching this function ensures that file reads happen only once, making
#         the UI much more responsive when checking individual subjects.
# MEASUREMENT: Confirmed via manual testing that subsequent subject checks are
#              instantaneous, whereas they previously had a noticeable lag.
@st.cache_data
def load_results_from_disk():
    """Load all results from the outputs/ directory, caching the result."""
    results = {}
    ...
    if agg.exists():
        try:
            ...
            results = {}  # Fallback to per-module files on failure
    else:
