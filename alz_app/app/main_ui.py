...
# --- Performance: cache file loading ---
# Caching this function prevents re-reading the files on every UI interaction.
@st.cache_data
def load_results_from_disk():
    """Load results from output files (aggregate JSON or per-module files)."""
    results = {}
    import json
    ...
    # try to load per-module summary files if aggregate is missing
    for name in ["mri_pet", "eeg", "adni", "tadpole", "proteomics"]:
        ...
        if summary_path.exists():
            try:
                ...
                pass # ignore if a single file fails
    return results
