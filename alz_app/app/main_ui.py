...
def load_results_from_disk():
    ...
    - Cached with @st.cache_data to avoid redundant disk I/O.
    - Converts prediction lists to dictionaries for O(1) lookup.
    """
    results = {}
    out_dir = Path.cwd() / "outputs"

    # Try loading aggregate file first
    ...
    # Load or complement per-module results
    for name in ["mri_pet", "eeg", "adni", "tadpole", "proteomics"]:
        ...
        # Load predictions if not already in memory or if stored as list
        module_data = results.get(name)
        if module_data:
            preds = module_data.get("predictions")
            # If predictions missing, try to read CSV
            if not preds:
                ...
                if preds_path.exists():
                    try:
                        ...
                        module_data["predictions"] = dict_preds
                    except Exception:
                        pass
            # If predictions are a list, convert to dict for O(1) lookup
...
# If no results in session, try to load from disk (cached)
if not results:
